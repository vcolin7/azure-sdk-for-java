...
public class CrcInputStream extends InputStream {
    private final static com.azure.core.util.logging.ClientLogger OLD_LOGGER =
        new com.azure.core.util.logging.ClientLogger(CrcInputStream.class);
        ...
        if (size == 0) {
            try {
                if (inputStream.read() != -1) {
                    ...
                    return FluxUtil.fluxError(OLD_LOGGER, new UnexpectedLengthException(String.format(
                        "Request body emitted %d bytes, more than the expected %d bytes.", totalLength, size),
                        totalLength, size));
                }
            } catch (IOException e) {
                return FluxUtil.fluxError(OLD_LOGGER, new UncheckedIOException(e));
            }
        }
        ...
        return Flux.defer(() -> {
            ...
            return Flux.generate(() -> inputStream, (is, sink) -> {
                ...
                if (numOfBytes == -1 && currentTotalLength[0] < size) {
                    sink.error(LOGGER.logThrowableAsError(new UnexpectedLengthException(String.format(
                ...
                if (currentTotalLength[0] >= size) {
                    try {
                        if (inputStream.read() != -1) {
                            ...
                            sink.error(LOGGER.logThrowableAsError(new UnexpectedLengthException(
                            ...
                            sink.error(LOGGER.logThrowableAsError(new IllegalStateException(
                                String.format("Read more data than was requested. Size of data read: %d. Size of data"
                                    + " requested: %d", currentTotalLength[0], size))));
                            return is;
                        }
                    } catch (IOException e) {
                        sink.error(LOGGER.logThrowableAsError(new RuntimeException("I/O errors occurred", e)));
                        return is;
                    }
                }
