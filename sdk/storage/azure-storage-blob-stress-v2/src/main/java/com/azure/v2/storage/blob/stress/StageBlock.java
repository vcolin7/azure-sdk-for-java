...
import com.azure.v2.storage.blob.stress.utils.OriginalContent;
...
public class StageBlock extends BlobScenarioBase<StorageStressOptions> {
    ...
    public StageBlock(StorageStressOptions options) {
        super(options);
    ...
    protected void runInternal(Context span) {
        ...
        try (CrcInputStream inputStream = new CrcInputStream(originalContent.getBlobContentHead(), options.getSize())) {
            ...
            blockBlobClient.stageBlockWithResponse(blockId, inputStream, options.getSize(), null, null, null, span);
